...
class PulseManagerLogic(QObject):
    ...
    def Run_experiment(self,value_loop,Type):
        ...
        for i in range(1,self.max_variations+1):
            ...
            for channel in self.channels:
                ...
                result=channel.a_experiment(i)
                list_channel_sequence=result[0]
                if list_channel_sequence!=None: 
                    Exp_i_pb.append(list_channel_sequence)
                    if max_end<result[1]:
                        max_end=result[1]
            max_end_times_vars.append(max_end)#we get the max end for each variation and add them to a list
            exp=Experiment(Exp_i_pb,i)# Now we see that each variation is one experiment, so a variation is a variation of the experiment
            exp.Prepare_Exp()#here we order the pulses of all the channels by time in the instace of the variation of the experiment
        ...
        if Type==0: 
            ...
            for m in range(0,len(max_end_times_vars)):
                timeout = timeout+(value_loop*max_end_times_vars[m]*1.2) #el el timepo total de cada variaicon por la cantidad de repeticciones por 1.2.
            print(f"timeout:{timeout}")
            counter=self.create_counter_task() #cretes the coutning conditions
            counter.start()#starts the task of counting
            self.a_Send_to_pulse_blaster(list_type_cero,value_loop,max_end_times_vars,divide_exp,counter)
            ...
            timeout= timeout*value_loop*1.2  #el tiempo total de una serie de variaicones * total de repeticiones*1,2
            print(f"timeout:{timeout}")
            counter=self.create_counter_task() #cretes the coutning conditions
            counter.start()#starts the task of counting
            # Flatten all the pulses into one list
            Flat_exp= [pulse for exp in self.Experiment_Hub for pulse in exp.pb_sequence]
            self.b_Send_to_Pulse_Blaster(Flat_exp,value_loop,max_end_1,divide_exp,counter)
        """
    ...
    def a_Send_to_pulse_blaster(self,Flat_exp,value_loop,max_end_times_vars,divided_value,counter): 
        """  here we must iterate each variation a number of value_loop times. we do this for all variations so.
        ...
        for d in range(0,len(divided_value)):
            ...
            for j in range(0,self.max_variations):
                ...
                spinapi.pb_start_programming(spinapi.PULSE_PROGRAM) #the pulse blaster stats writting th einstructiuons we send it
                start=spinapi.pb_inst_pbonly(int(sum(Flat_exp[j][0].channel_binary[0])),Inst.LOOP,value_loop,(Flat_exp[j][0].end_tail-Flat_exp[j][0].start_tail)*spinapi.us) # generates a loop of instruction here only one iteration
                print(f"Flat_exp[0].channel_binary:{Flat_exp[j][0].channel_binary[0]}")
                print(f"spinapi.pb_inst_pbonly({sum(Flat_exp[j][0].channel_binary[0])},Inst.LOOP,{value_loop},({Flat_exp[j][0].end_tail-Flat_exp[j][0].start_tail})*spinapi.us)") 
                for i in range(1,len(Flat_exp[j])):# we start from one because we already did the 0 index
                    if i!=len(Flat_exp) -1: #maybe we need to take the -1
                        print(f"spinapi.pb_inst_pbonly({sum(list(Flat_exp[j][i].channel_binary[0]))},Inst.CONTINUE,0,({Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail})*spinapi.us)")
                        spinapi.pb_inst_pbonly(int(sum(Flat_exp[j][i].channel_binary[0])),Inst.CONTINUE,0,(Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail)*spinapi.us)
                    else:
                        print(f"spinapi.pb_inst_pbonly({sum(list(Flat_exp[j][i].channel_binary[0]))},Inst.CONTINUE,0,({Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail})*spinapi.us)")
                        spinapi.pb_inst_pbonly(int(sum(Flat_exp[j][i].channel_binary[0])),Inst.CONTINUE,0,(Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail)*spinapi.us)
                        print(f"spinapi.pb_inst_pbonly({sum(list(Flat_exp[j][i].channel_binary[0]))},Inst.END_LOOP,start,{Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail}")
                        spinapi.pb_inst_pbonly(int(sum(Flat_exp[j][i].channel_binary[0])),Inst.END_LOOP,start,Flat_exp[j][i].end_tail-Flat_exp[j][i].start_tail)
                ...
                self.busy_wait_us(time_wait)  # Intended wait: minimum wait time until the next variation 
                end = time.perf_counter()
                print(f"Actual wait: {(end - start)*1e6:.2f} µs") #to get a glimpse of the error in wait time
    ...
    def b_Send_to_Pulse_Blaster(self,Flat_exp,max_end_1,divided_value,counter):
        ...
        for d in range(0,len(divided_value)):
            ...
            spinapi.pb_start() #here we start the spinapi
            start = time.perf_counter()
            self.busy_wait_us(max_end_1*value_loop)  # Intended wait: minimum wait time until the next variation 
            end = time.perf_counter()
            print(f"Actual wait: {(end - start)*1e6:.2f} µs") #to get a glimpse of the error in wait time
            #el el timepo total que espera el counter para seguir a la siguient variacion. Durante ese tiempo se toman todo los datos de una variacion. Aqui se debe calcular el maximo tiempo de cada variacion y multiplicar por value _loop 
            print(f"value_loop:{value_loop}")
